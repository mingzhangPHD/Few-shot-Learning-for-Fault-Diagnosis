...
class SimpleDataset(Dataset):
    def __init__(self, data_file, data_type):
        if data_type == 'origi':
            self.X_orig = np.array(pd.read_hdf(data_file, key='X_w'))
            self.Y_orig = np.array(pd.read_hdf(data_file, key='Y'))
        elif data_type == 'fft':
            self.X_orig = np.array(pd.read_hdf(data_file, key='X_fft'))
            self.Y_orig = np.array(pd.read_hdf(data_file, key='Y'))
        else:
            raise ValueError()
...
class MetaDataset(Dataset):
    def __init__(self, data_file, batch_size, data_type):
        if data_type == 'origi':
            X_orig = np.array(pd.read_hdf(data_file, key='X_w'))
            Y_orig = np.array(pd.read_hdf(data_file, key='Y'))
        elif data_type == 'fft':
            X_orig = np.array(pd.read_hdf(data_file, key='X_fft'))
            Y_orig = np.array(pd.read_hdf(data_file, key='Y'))
        else:
            raise ValueError()
