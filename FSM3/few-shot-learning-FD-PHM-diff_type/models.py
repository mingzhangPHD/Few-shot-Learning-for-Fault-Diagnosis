...
class MatchingNetwork(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.optimizer = optim.Adam(params=self.param_list, lr=0.001)
...
class MatchingNetwork_feat_space(nn.Module):
    ...
    def __init__(self, args):
        ...
        flatten_pre = Flatten()
        fc_pre = nn.Linear(64*self.args.conv_final_size, self.args.pretrain_source_num_classes)
        ...
        fc_metric = nn.Linear(64 * self.args.conv_final_size, self.args.backbone_out_dim)
        ...
        self.optimizer_classify_model = optim.Adam(params=self.param_classify_model, lr=0.001)
        self.optimizer_metric_model = optim.Adam(params=self.param_metric_backbone, lr=0.001)
        # self.optimizer_metric_model = optim.SGD(params=self.param_metric_backbone, lr=0.001)
    ...
    def save_feature(self, data_loader, feat_save_path, save_num=-1, type='Train'):
        ...
        for i, (x, y) in enumerate(data_loader):
            ...
            # feat1 = self.metric_backbone(backbone_feat)
        ...
        f = open(os.path.join(feat_save_path, '{}_{}_{}shot_{}_feat.pkl'.format(self.args.dataset, self.args.transfer_type, self.args.n_shot, type)), 'wb')
...
class Baseline_FeatureKnn_Proto(nn.Module):
    # basic CNN classify model, with two classifiers in it. Used for source task and target task with different num of types
    # we only use the trained feature extractor and classify the query data by matching to support data (KNN classifier)
    def __init__(self, args):
        super(Baseline_FeatureKnn_Proto, self).__init__()
        self.args = args
    ...
    def test_iter_meta_finetune(self, data_loader, epoch):
        ...
        for i, (x, y) in enumerate(data_loader):
            ...
            y_support = y_support.contiguous().view(n_way * self.args.n_shot, 1)  # [n_way*n_shot, 1]
            y_query = y_query.contiguous().view(n_way * self.args.n_query, 1).squeeze(1)  # [n_way*n_query]
    ...
    def knn_matching(self, x_support, x_query, y_support, n_way):
        x_feat_support = self.backbone(x_support)  # [n_way*n_shot, f_dim]
        x_feat_query = self.backbone(x_query)  # [n_way*n_query, f_dim]

        x_feat_support = x_feat_support.contiguous().view(n_way, -1, x_feat_support.size()[-1])  # [n_way, n_shot, f_dim]
        x_feat_support = x_feat_support.mean(1)  # [n_way, f_dim]
        y_support = y_support.contiguous().view(n_way, -1, 1)  # [n_way, n_shot, 1]
        y_support = y_support[:, 0, :]  # [n_way, 1]
        ...
        # get support label in one-hot form
        y_support_onehot = torch.zeros((y_support.size()[0], n_way)).scatter_(1, y_support.cpu(), 1)
        y_support_onehot = y_support_onehot.to(self.gpu_device)  # [n_way, n_way]
...
class ProtoNetwork(nn.Module):
    # perform episodic proto network training in original data space
    def __init__(self, args):
    ...
    def forward_no_fce(self, x):
        ...
        # get support label in one-hot form
        y_support = torch.from_numpy(np.array(range(n_way)))
        y_support_onehot = torch.zeros((n_way, n_way)).scatter_(1, y_support.unsqueeze(1), 1)
        y_support_onehot = y_support_onehot.to(self.gpu_device)  # [n_way*n_shot, n_way]
...
class ProtoNetwork_feat_space(nn.Module):
    # perform episodic matching network training in pretrained feature space
    def __init__(self, args):
        super(ProtoNetwork_feat_space, self).__init__()
        self.args = args
...
class ProtoNetwork_pretrain(nn.Module):
    # pretrain backbone as initial of matching network
    def __init__(self, args):
        super(ProtoNetwork_pretrain, self).__init__()
        self.args = args
